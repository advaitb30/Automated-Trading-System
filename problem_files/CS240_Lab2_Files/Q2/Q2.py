...
def l2_loss(X, Y, w):
    ...
    loss = 1/len(X)*((torch.sum(torch.pow((torch.matmul(X, w) - Y), 2))))
...
def l2_loss_derivative(X, Y, w):
    ...
    derivative = 1/len(X)*torch.matmul(X.T, (torch.matmul(X, w) - Y))
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    '''
    while (abs(old_loss - l2_loss(X_train, Y_train, w)) > epsilon):    
        old_loss = l2_loss(X_train, Y_train, w)
        dw = l2_loss_derivative(X_train, Y_train, w)
        w -= eta*dw
        test_err.append(old_loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('dataset', type=str, help='dataset02.csv')
    ...
    data = pd.read_csv(args.dataset , index_col=0, delimiter=',', encoding='utf-8')
    ...
    for degree in possible_degrees:
        ...
        if(degree == 1):
            loss_min = l2_test_loss
        if(l2_test_loss < loss_min):
            min_deg = degree
        print(degree, l2_loss_train, l2_test_loss)

    optimal_degree = min_deg
